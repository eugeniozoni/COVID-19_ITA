...
set.seed(1)
pred_ITA_new_after <- project(i_ITA[16:31], R = median(growth_R0_after_peak), si = si_ITA_n, n_days = 10, n_sim = 1000)
pred_ITA_new_after
plot(pred_ITA_new_after) + labs(title="Future Incidence (R = 0.69) - based on new positive cases growth after shutdown")

set.seed(1)
pred_ITA_new_after_peak <- project(i_ITA[16:31], R = median(growth_R0_after), si = si_ITA_n, n_days = 10, n_sim = 1000)
pred_ITA_new_after_peak
plot(pred_ITA_new_after_peak) + labs(title="Future Incidence (R = 1.4) - based on new positive cases growth after peak")
...
plot(pred_ITA_total_before) + labs(title="Future Incidence (R = 2.6) - based on growth of total positive cases in the first 15 days of diffusion")

set.seed(1)
pred_ITA_total_after <- project(i_total_ITA_ng[16-30], R = median(growth_R0_after_total), si = si_ITA_n, n_days = 10, n_sim = 1000)
pred_ITA_total_after
plot(pred_ITA_new_after) + labs(title="Future Incidence (R = 1.7) - based on total positive cases growth after shutdown")
```
...
Also for the importance and significance of estimating the effective reproduction number we follow the approach proposed by Tim Chrurches and described here: https://timchurches.github.io/blog/posts/2020-02-18-analysing-covid-19-2019-ncov-outbreak-data-with-r-part-1/ 
...
Information related to the session can be retrieved here.
```{r}
sessionInfo()
```
